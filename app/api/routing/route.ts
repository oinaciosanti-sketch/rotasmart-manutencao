...
export async function POST(request:Request){
 ...
 try{
  ...
  const route=payload?.routes?.[0],legs=Array.isArray(route?.legs)?route.legs:[],summary=route?.summary;
  const lengthInMeters=Number.isFinite(summary?.lengthInMeters)?Number(summary.lengthInMeters):legs.reduce((sum:number,leg:any)=>sum+(Number.isFinite(leg?.summary?.lengthInMeters)?Number(leg.summary.lengthInMeters):0),0);
  const travelTimeInSeconds=Number.isFinite(summary?.travelTimeInSeconds)?Number(summary.travelTimeInSeconds):legs.reduce((sum:number,leg:any)=>sum+(Number.isFinite(leg?.summary?.travelTimeInSeconds)?Number(leg.summary.travelTimeInSeconds):0),0);
  ...
  if(!Number.isFinite(lengthInMeters)||!Number.isFinite(travelTimeInSeconds)||lengthInMeters<=0||travelTimeInSeconds<=0||points.length<2){const detail=points.length>=2?"TomTom retornou geometria, mas o resumo de distância/tempo não foi interpretado.":"Resposta sem geometria válida.";logAttempt({keyConfigured:true,coordinateCount:coordinates.length,providerHttpStatus:response.status,providerMessage:detail,fallbackUsed:true});return fallback(coordinates,"route_not_found",detail,response.status)}
  ...
  return NextResponse.json({source:"tomtom",distanceKm:lengthInMeters/1000,durationMinutes:Math.max(1,Math.round(travelTimeInSeconds/60)),geometry:points,fallback:false,calculatedAt,diagnostics:{keyConfigured:true,coordinateCount:coordinates.length,providerHttpStatus:response.status,fallbackUsed:false}});
