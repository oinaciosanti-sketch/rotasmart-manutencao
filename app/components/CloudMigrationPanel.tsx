...
export default function CloudMigrationPanel() {
  ...
  return <div className="panel settings-card cloud-card"><div className="cloud-title"><div><Database/></div><span><h3>Banco de dados Supabase</h3><p>{cloudStatus}</p></span><button className="icon-button" onClick={refresh} aria-label="Atualizar status"><RefreshCw/></button></div>{!configured?<div className="cloud-local"><Cloud/><div><b>Modo local ativo</b><span>Adicione as variáveis do Supabase para habilitar login e sincronização.</span></div></div>:<div className={`cloud-health ${status?.reachable?"ok":"warning"}`}>{status?.reachable?<Check/>:<AlertTriangle/>}<span>{status?.message||"Verificando conexão…"}</span></div>}{configured&&user&&<><div className="cloud-account"><span>Usuário</span><b>{profile?.nome||user.email}</b><small>{profile?.role||"analista"}</small></div><button className="btn primary" onClick={start} disabled={busy||profile?.role==="visualizador"}><Cloud/>Migrar dados locais para o banco</button>{profile?.role==="visualizador"&&<p className="warning-text">O perfil visualizador não pode gravar dados.</p>}</>}{preview&&<div className="migration-preview"><b>Resumo da migração</b><div><span>Chamados<strong>{preview.tickets}</strong></span><span>Analistas<strong>{preview.analysts}</strong></span><span>Técnicos<strong>{preview.technicians}</strong></span><span>Filiais<strong>{preview.branches}</strong></span><span>Rotas<strong>{preview.routes}</strong></span></div><p>Duplicidades serão atualizadas pelas chaves naturais. Nenhum dado local será apagado.</p><div className="settings-actions"><button className="btn secondary" onClick={()=>setPreview(null)}>Cancelar</button><button className="btn primary" onClick={migrate}>Confirmar migração</button></div></div>}{busy&&<div className="migration-progress"><Loader2 className="spin"/><span>{progress}</span></div>}{report&&totals&&<div className="migration-report"><b>Dados migrados. O backup local foi mantido.</b><span>{totals.created} criados · {totals.updated} atualizados · {totals.ignored} ignorados · {totals.errors} erros</span></div>}{error&&<p className="warning-text"><AlertTriangle/>{error}</p>}</div>;
}
