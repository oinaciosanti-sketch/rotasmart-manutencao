...
export default class RouteErrorBoundary extends React.Component<{children:React.ReactNode},{hasError:boolean}>{
 ...
 private clearLocalData=()=>{if(!window.confirm("Limpar todos os dados locais do RotaSmart? Faça isso somente se você já tiver um backup."))return;try{Object.keys(localStorage).filter(key=>key.startsWith("rotasmart-")).forEach(key=>localStorage.removeItem(key));}catch{} window.location.reload();};
