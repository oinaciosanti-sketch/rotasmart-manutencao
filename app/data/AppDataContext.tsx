...
export type AppTicket={id:number;number:string;branch:string;branchId?:number;branchNumber?:string;city:string;uf?:string;address:string;latitude?:number|null;longitude?:number|null;desc:string;status:"Novo"|"Programado"|"Aguardando compra"|"Aguardando entrega"|"Em atendimento"|"Concluído"|"Fechado";urgency:"Alta"|"Média"|"Baixa";tech?:string;date?:string;duration:number;analyst?:string;openedAt?:string;dueAt?:string;notes?:string;routeOrder?:number;routeId?:string;planningStatus?:"nao_planejado"|"em_rota_rascunho"|"rota_confirmada";reviewBranch?:boolean};
...
function mergeBranches(initial:Filial[],value:unknown):Filial[]{
 ...
 const merged=initial.map(base=>{const index=saved.findIndex((local,i)=>{if(used.has(i))return false;if(base.cnpj&&local.cnpj)return base.cnpj===local.cnpj;return (!base.cnpj||!local.cnpj)&&base.numeroFilial===local.numeroFilial});if(index<0)return base;used.add(index);return {...base,...saved[index]};});
 const localOnly=saved.filter((_,index)=>!used.has(index));
 return [...merged,...localOnly].map((branch,index)=>({...branch,id:index+1}));
}
