...
export type AppTicket={id:number;number:string;branch:string;branchNumber?:string;city:string;desc:string;status:"Novo"|"Programado"|"Aguardando compra"|"Aguardando entrega"|"Em atendimento"|"Concluído"|"Fechado";urgency:"Alta"|"Média"|"Baixa";tech?:string;date?:string;duration:number;address:string;analyst?:string;openedAt?:string;dueAt?:string;notes?:string;routeOrder?:number;routeId?:string;planningStatus?:"nao_planejado"|"em_rota_rascunho"|"rota_confirmada";reviewBranch?:boolean};
...
type AppData={tickets:AppTicket[];technicians:AppTechnician[];branches:Filial[];setTickets:React.Dispatch<React.SetStateAction<AppTicket[]>>;setTechnicians:React.Dispatch<React.SetStateAction<AppTechnician[]>>;setBranches:React.Dispatch<React.SetStateAction<Filial[]>>;clearLocalData:()=>void};
...
export function AppDataProvider({initialTickets,initialTechnicians,initialBranches,children}:{initialTickets:AppTicket[];initialTechnicians:AppTechnician[];initialBranches:Filial[];children:React.ReactNode}){
 ...
 return <Context.Provider value={{tickets,technicians,branches,setTickets,setTechnicians,setBranches,clearLocalData}}>{children}</Context.Provider>;
}
