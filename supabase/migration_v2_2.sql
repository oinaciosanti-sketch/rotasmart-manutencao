...
alter table public.tickets add column if not exists technician_status text;
...
begin
  ...
  if p_action='start' then update public.tickets set service_started_at=coalesce(service_started_at,now()),technician_status='em_atendimento',status='Em atendimento',technician_notes=coalesce(p_notes,technician_notes),updated_by=auth.uid() where id=p_ticket_id returning * into result;
  elsif p_action='finish' then select * into result from public.tickets where id=p_ticket_id;if result.service_started_at is null then raise exception 'Inicie o atendimento antes de finalizar.';end if;update public.tickets set service_finished_at=now(),technician_status='atendimento_finalizado',technician_notes=coalesce(p_notes,technician_notes),updated_by=auth.uid() where id=p_ticket_id returning * into result;
  elsif p_action='complete' then update public.tickets set service_finished_at=coalesce(service_finished_at,now()),technician_status='concluido',status='Concluído',completed_by_technician_at=now(),technician_notes=coalesce(p_notes,technician_notes),updated_by=auth.uid() where id=p_ticket_id returning * into result;
  elsif p_action='pending' then if coalesce(trim(p_notes),'')='' then raise exception 'Observação obrigatória para pendenciar.';end if;update public.tickets set technician_status='pendente',technician_notes=p_notes,technician_pending_reason=p_reason,status=case when p_reason='Aguardando compra' then 'Aguardando compra' else 'Aguardando entrega' end,updated_by=auth.uid() where id=p_ticket_id returning * into result;
